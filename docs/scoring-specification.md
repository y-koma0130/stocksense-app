...
| 短期平均出来高 | 罠株除外、出来高急増スコア | Yahoo Finance API | 週次/月次 |
| 長期平均出来高 | 出来高急増スコア（中期のみ） | Yahoo Finance API | 週次 |
...
**中期スコア計算式**:
```
総合スコア = (PERスコア × 24% + PBRスコア × 18% + RSIスコア × 16% + 価格位置スコア × 12% + RSIモメンタムスコア × 18% + 出来高急増スコア × 12%) / 100
```
...
PER: 24%           # 割安性評価
PBR: 18%           # 割安性評価
RSI（静的）: 16%   # 売られすぎ評価（14週RSI）
価格位置: 12%      # 底値圏評価（26週高安レンジ）
RSIモメンタム: 18% # 反発初動検出（短期RSI - 長期RSI）
出来高急増: 12%    # 注目度上昇検出（短期/長期出来高比）
合計: 100%
```

**中期の特徴**:
- RSIモメンタム（短期RSI 2週 - 長期RSI 14週）で**反発の初動**を検出
- 出来高急増（短期出来高 / 長期出来高）で**市場の注目度上昇**を検出
- 「割安かつ反発し始めた銘柄」「注目が集まり始めた銘柄」を高く評価
...
### 6. 出来高急増スコア（注目度上昇検出）【中期のみ】

**概要**: 短期平均出来高と長期平均出来高の比率から、市場の注目度上昇を検出

**データソース**: Yahoo Finance `historical` モジュールから日足株価データを取得

**期間**:
- 短期平均出来高: 直近1週間（5営業日）の平均
- 長期平均出来高: 直近5週間（25営業日）の平均

**計算ロジック（線形補間 0-100点）**:
```typescript
// 出来高比率 = 短期平均出来高 / 長期平均出来高
ratio = avgVolumeShort / avgVolumeLong

// 線形補間によるスコア計算
if (ratio >= 2.0)        → 100点  // 大幅増加（2倍以上）
if (ratio = 1.0)         → 50点   // 横ばい
if (ratio <= 0.5)        → 0点    // 大幅減少（半分以下）

// 中間値の線形補間:
// ratio 1.0〜2.0: 50点〜100点
// ratio 0.5〜1.0: 0点〜50点
```
...
**除外条件**:
- 短期平均出来高 = null → 50点（中央値）
- 長期平均出来高 = null → 50点（中央値）
- 長期平均出来高 = 0 → 50点（計算不可）

---

### 7. EPS成長率スコア（利益成長性評価）【長期のみ】
...
#### 判定の考え方
- **出来高（流動性）**: 出来高が少ないと売買が困難で、スリッページが大きくなるリスクがある。市場規模に応じた閾値を設定
...
**RSIモメンタムスコア**:
```
momentum = 42 (2週RSI) - 35 (14週RSI) = +7
→ -30〜+30の範囲を0〜100にマッピング
→ score = ((7 + 30) / 60) × 100 ≈ 62点
```
...
→ 1.0〜2.0の範囲なので線形補間
→ score = 50 + ((1.5 - 1.0) / (2.0 - 1.0)) × 50 = 75点
```
...
2. filterProMarket (ドメインサービス)
   - PROマーケット銘柄を除外
   ↓
3. isTrapStock (ドメインサービス)
   - 罠銘柄（財務健全性が低い銘柄、出来高が低い銘柄）を除外
   - 市場別の閾値で判定
   ↓
4. calculateValueStockScore (ドメインサービス)
   - 各銘柄のスコアを計算
   - 中期: PER, PBR, RSI, 価格位置, RSIモメンタム, 出来高急増
   - 長期: PER, PBR, RSI, 価格位置, EPS成長率
   - configの閾値と重みを使用
   - 市場別補正を適用
   ↓
...
```
calculateValueStockScore (ドメインサービス)
├── calculatePERScore
│   └── 業種平均比率 → 線形補間 → 市場別補正
├── calculatePBRScore
│   └── 業種平均比率 → 線形補間 → 市場別補正
...
│   └── 価格位置 → 線形補間 → 市場別補正
├── calculateRSIMomentumScore（中期のみ）
│   └── 短期RSI - 長期RSI → 線形補間
├── calculateVolumeSurgeScore（中期のみ）
│   └── 短期出来高 / 長期出来高 → 線形補間
├── calculateEpsGrowthScore（長期のみ）
│   └── 3年CAGR → 線形補間
└── 重み付け合計 → 総合スコア
```
...
```typescript
{
  ...
  perWeight: 24,           // PERスコア
  pbrWeight: 18,           // PBRスコア
  rsiWeight: 16,           // RSI静的スコア
  priceRangeWeight: 12,    // 価格位置スコア
  rsiMomentumWeight: 18,   // RSIモメンタムスコア（中期のみ）
  volumeSurgeWeight: 12,   // 出来高急増スコア（中期のみ）
...
VOLUME_TRAP_THRESHOLDS = {
  PRIME: 30000,     // プライム市場の下限
  STANDARD: 7000,   // スタンダード市場の下限
  GROWTH: 5000,     // グロース市場の下限
  OTHER: 7000,      // その他市場の下限
}

// 出来高急増スコアの閾値設定（線形補間用）
VOLUME_SURGE_CONFIG = {
  MAX_RATIO: 2.0,      // 100点を付与する出来高比率
  NEUTRAL_RATIO: 1.0,  // 50点を付与する出来高比率（横ばい）
  MIN_RATIO: 0.5,      // 0点を付与する出来高比率
  NEUTRAL_SCORE: 50,   // データ不足時のスコア
}

// 出来高計算の期間設定（週数）
VOLUME_PERIOD = {
  SHORT: 1,  // 短期平均の計算期間（1週間）
  LONG: 5,   // 長期平均の計算期間（5週間）
}
```
...
### 2025-12-02 (最新)
- **中期スコアに出来高急増スコアを追加**: 短期/長期出来高比で市場注目度を評価
- **罠株除外に出来高基準を追加**: 市場別に流動性チェック（プライム≤30,000、スタンダード≤7,000、グロース≤5,000株/日以下で除外）
- **長期にも出来高データを追加**: 罠株除外の出来高チェックを中期・長期両方で実施
- **重み配分を更新**:
  - 中期: PER 24%, PBR 18%, RSI静的 16%, 価格位置 12%, RSIモメンタム 18%, **出来高急増 12%**
  - 長期: PER 28%, PBR 25%, RSI静的 15%, 価格位置 12%, EPS成長率 20%（変更なし）
- **出来高急増スコアを線形補間に統一**: ratio 0.5→0点、1.0→50点、2.0→100点の線形スケール
- **volumeConfig.tsを追加**: 出来高関連の設定値を一元管理
