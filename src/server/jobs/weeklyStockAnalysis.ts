...
import { collectMidTermAnalysisTargetStocks } from "./utils/collectAnalysisTargetStocks";

export const weeklyStockAnalysis = inngest.createFunction(
  {
    id: "weekly-stock-analysis",
  ...
  async ({ step }) => {
    ...
    const marketAnalysis = await step.run("fetch-market-analysis", async () => {
      return await getLatestMarketAnalysis({ periodType: "mid_term" });
    });

    // 分析対象銘柄を収集（全銘柄上位 + ユーザーフィルター上位）
    const topStocks = await step.run("collect-analysis-target-stocks", async () => {
      return await collectMidTermAnalysisTargetStocks();
    });
    ...
    return {
      message: `Weekly stock analysis completed for ${results.length} stocks`,
      periodType: "mid_term",
